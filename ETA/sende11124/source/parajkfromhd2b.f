c mpec2.1 version eta. For copyright and Disclaimers, see start.f
      subroutine parajkfromhd2b(ipair,ngaus,ibdat,nhcolt,ihmat,nocc,icol9d20s16
     ...
     $     isblkder1,nsblkder1,isblkkder,nsblkkder,iprop,nbasisp,xgoal, 11d10s22
     $     bc,ibc,idorel)                                               3d6s23
      implicit real*8 (a-h,o-z)
c
c     like jkfromhd1, working with (a'b'|dc) instead of (a'b|dc)
c
      external second
...
c
      ...
      if(mynprocg.gt.idp)then                                           11d30s12
       write(6,*)('in parajkfromhd2b, mynprocg = '),mynprocg
 ...
 5653          format(4i8)
               if(ibb.le.iaa.and.isa.le.isb)then                        9d27s16
                if(issb.eq.isb.and.issa.eq.isa)then                      9d20s16
                 if(ipass.eq.1)then                                      2d11s13
                  nrtotal=nrtotal+ntmpmat*nhere                         3d6s23
                  ...
                  do in=0,ntmpmat-1                                     3d6s23
                   do j=0,nhere-1                                         3d2s16
                    iad=myh(iib)+ix+nxn*(j+nhere*in)                    3d6s23
                 ...
                 if(ipass.eq.1)then
                  nrtotal=nrtotal+ntmpmat*nhere                         3d6s23
                  irecv(ip+1)=irecv(ip+1)+nhere*ntmpmat                 3d6s23
                  myh(iib)=myh(iib)+nhere*ntmpmat                       3d6s23
                  nsumx=nsumx+1
                 else                                                   9d20s16
       ...
       if(ipass.eq.1)then                                               11d28s12
        ...
        if(nrecv.gt.ngot)then                                           11d30s12
         ...
         call enough('parajkfromhd2b.  1',bc,ibc)
        end if                                                          11d30s12
        ibufs=ibcoff                                                    11d28s12
        ibcoff=ibufs+max(nsend,nrecv)                                   11d30s12
        call enough('parajkfromhd2b.  2',bc,ibc)
...
c
         ...
         if(ncd.gt.0.and.iis.gt.0)then                                  12d21s12
          ...
          if(nhere.gt.0)then                                            11d30s12
           if(isa.eq.isb)then                                           9d20s16
            ...
            itmp3=itmp2+nbasdwsa*nbasdwsb                               3d7s23
            ibcoff=itmp3+nbasdwsa*nbasdwsb                              3d7s23
            call enough('parajkfromhd2b.  3',bc,ibc)
            ...
            jtmp3=itmp3-1                                               3d7s23
            do icol=1,nhere                                             11d30s12
             jorb=iorbx(isb)                                            3d6s23
             factr=0d0                                                   10d30s20
             do in=0,ntmpmat-1                                          3d6s23
              ij=0
              do i=1,nbasdwsa                                            8d31s15
               do j=1,i                                                  11d30s12
                iad0=myh(iis)+ij+nxn*(icol-1+nhere*in)                  3d6s23
                ij=ij+1                                                  2d12s13
              ...
              call dgemm('n','n',nbasdwsa,nbasdwsc(isa),nbasdwsa,        4d7s22
     $            1d0,bc(itmp),nbasdwsa,bc(jorb),nbasdwsa*ncomp,        3d6s23
     $            0d0,bc(itmp2),nbasdwsa,                               8d31s15
     d' parajkfromhd2b.  1')
              do i=1,nbasdwsc(isa)                                       4d7s22
               do j=1,nbasdwsa                                           8d31s15
                iad1=itmp2+j-1+nbasdwsa*(i-1)                            8d31s15
                iad2=itmp+i-1+nbasdwsc(isa)*(j-1)                        4d7s22
                bc(iad2)=bc(iad1)                                        11d30s12
               end do                                                    11d30s12
              end do                                                     11d30s12
              call dgemm('n','n',nbasdwsc(isa),nbasdwsc(isa),nbasdwsa,   4d7s22
     $            2d0,bc(itmp),nbasdwsc(isa),bc(jorb),nbasdwsa*ncomp,   3d6s23
     $            factr,bc(itmp3),nbasdwsc(isa),                        3d7s23
     d' parajkfromhd2b.  2')
              factr=1d0                                                 3d6s23
              jorb=jorb+nbasdwsa                                        3d6s23
             end do                                                     3d6s23
             do i=1,nbasdwsc(isa)                                       4d7s22
              do j=1,i                                                  11d30s12
               iad0=ihcol(iis)+((i*(i-1))/2)+j-1+nxnc*(icol-1)          4d12s22
               iad1=jtmp3+j+nbasdwsc(isa)*(i-1)                         3d7s23
            ...
            call enough('parajkfromhd2b.  4',bc,ibc)
            ...
            do icol=1,nhere                                              11d30s12
             ...
             do in=0,ntmpmat-1                                           10d30s20
              iad=myh(iis)+nxn*(icol-1+nhere*in)                        3d6s23
     ...
     $            1d0,bc(iad),nbasdwsa,bc(jorbb),nbasdwsb*ncomp,        3d6s23
     ...
     $            2d0,bc(iad),nbasdwsc(isb),bc(jorba),nbasdwsa*ncomp,   3d6s23
...
c
c     for (oo|oo) go for (o'o'|oo) and (oo|o'o')
c     for (oo|ov) go for (o'o'|ov) and (oo|o'v')
c     recall we have 2(v'v'|ov)
...
c     (o'o'|oo) part ok
c
       itry=iptoh(isblkder1(4,isb),isblkder1(3,isb),isblkder1(2,isb))
       itry2=iptoh(isblkder1(4,isb),isblkder1(3,isb),isblkder1(1,isb))
       itry3=iptoh(isblkder1(3,isb),isblkder1(4,isb),isblkder1(2,isb))
       itry4=iptoh(isblkder1(3,isb),isblkder1(4,isb),isblkder1(1,isb))
     ...
     $      mynprocg,mynowprog,ilh,ihh,i1s,i1e,i2s,i2e)
        ...
        ii0=ihcol(itry)
        if(isblkder1(1,isb).eq.isblkder1(2,isb))then
         nrowh=(nbasdwsc(isblkder1(1,isb))                              9d20s16
     $        *(nbasdwsc(isblkder1(1,isb))+1))/2                        9d20s16
        else                                                            9d20s16
        ...
        do ic=i2s,i2e
         ...
         do id=i10,i1n
          ...
          if(ic.le.nocc(isblkder1(3,isb)))then                          9d28s16
           if(isblkder1(1,isb).eq.isblkder1(2,isb))then                 9d28s16
            do ib=0,nocc(isblkder1(2,isb))-1
             do ia=0,nocc(isblkder1(1,isb))-1
              ix=max(ia,ib)+1                                           9d28s16
              in=min(ia,ib)                                             9d28s16
              ix=((ix*(ix-1))/2)+in
              iadh=ii0+ix                                                9d20s16
              iadj=i4od2b(isb)+ia+nocc(isblkder1(1,isb))*(ib                 9d28s16
     $       +nocc(isblkder1(2,isb))*(icm+nocc(isblkder1(3,isb))*idm))  9d28s16
        ...
        do ic=i2s,i2e
         ...
         do id=i10,i1n
          ...
          if(ic.le.nocc(isblkder1(3,isb)))then                          9d28s16
           do ib=0,nocc(isblkder1(2,isb))-1
            do ia=0,nocc(isblkder1(1,isb))-1
             iadh=ii0+ib+nbasdwsc(isblkder1(2,isb))*ia
             iadj=i4od2b(isb)+ia+nocc(isblkder1(1,isb))*(ib
     $       +nocc(isblkder1(2,isb))*(icm+nocc(isblkder1(3,isb))*idm))
             bc(iadj)=bc(iadj)+bc(iadh)                                 10d31s16
            end do
       ...
       else if(itry3.gt.0)then                                          9d28s16
     ...
     $       +nocc(isblkder1(2,isb))*(idm+nocc(isblkder1(3,isb))*icm))
       ...
       else if(itry4.gt.0)then                                          9d28s16
        call ilimts(nocc(isblkder1(3,isb)),nbasdwsc(isblkder1(4,isb)),  9d28s16
     $      mynprocg,mynowprog,ilh,ihh,i1s,i1e,i2s,i2e)
        i10=i1s
        i1n=nocc(isblkder1(3,isb))
...
c
       ...
       itry3=iptoh(isblkder1(1,isb),isblkder1(2,isb),isblkder1(4,isb))
       itry4=iptoh(isblkder1(1,isb),isblkder1(2,isb),isblkder1(3,isb))
       if(itry.gt.0)then                                                9d16s16
     ...
     $      mynprocg,mynowprog,ilh,ihh,i1s,i1e,i2s,i2e)
        ...
        ii0=ihcol(itry)
        if(isblkder1(3,isb).eq.isblkder1(4,isb))then
         nrowh=(nbasdwsc(isblkder1(3,isb))                              9d20s16
     $        *(nbasdwsc(isblkder1(3,isb))+1))/2                        9d20s16
        else                                                            9d20s16
           ...
           if(isblkder1(1,isb).eq.isblkder1(2,isb))then                 9d28s16
            do ib=0,nocc(isblkder1(4,isb))-1
             do ia=0,nocc(isblkder1(3,isb))-1
              ...
              ix=((ix*(ix-1))/2)+in
              iadh=ii0+ix                                                9d20s16
              iadj=i4od2b(isb)+icm+nocc(isblkder1(1,isb))*(idm          9d28s16
     $       +nocc(isblkder1(2,isb))*(ia+nocc(isblkder1(3,isb))*ib))    9d28s16
              bc(iadj)=bc(iadj)+bc(iadh)                                10d31s16
             end do
            end do
           else                                                          9d20s16
        ...
        do ic=i2s,i2e
         ...
         do id=i10,i1n
          ...
          if(ic.le.nocc(isblkder1(1,isb)))then                          9d28s16
           do ib=0,nocc(isblkder1(4,isb))-1
            do ia=0,nocc(isblkder1(3,isb))-1
             iadh=ii0+ib+nbasdwsc(isblkder1(4,isb))*ia
             iadj=i4od2b(isb)+icm+nocc(isblkder1(1,isb))*(idm
     $       +nocc(isblkder1(2,isb))*(ia+nocc(isblkder1(3,isb))*ib))
             bc(iadj)=bc(iadj)+bc(iadh)                                 10d31s16
            end do
     ...
     $       +nocc(isblkder1(2,isb))*(ia+nocc(isblkder1(3,isb))*ib))
       ...
       if(idwsdeb.gt.10)then
         write(6,*)('for 4od2 after parajkfromhd2b')
...
c
     ...
     $      mynprocg,mynowprog,ilh,ihh,i1s,i1e,i2s,i2e)
        ...
        ii0=ihcol(itry)
        if(isblkxder1(1,isb).eq.isblkxder1(2,isb))then
         nrowh=(nbasdwsc(isblkxder1(1,isb))                              9d20s16
     $        *(nbasdwsc(isblkxder1(1,isb))+1))/2                        9d20s16
        else                                                            9d20s16
        ...
        do ic=i2s,i2e
         ...
         do id=i10,i1n
          ...
          if(ic.gt.nocc(isblkxder1(4,isb)))then                          9d28s16
           if(isblkxder1(1,isb).eq.isblkxder1(2,isb))then                 9d28s16
            do ib=0,nocc(isblkxder1(2,isb))-1
             do ia=0,nocc(isblkxder1(1,isb))-1
              ...
              ix=((ix*(ix-1))/2)+in
              iadh=ii0+ix                                                9d20s16
              iadj=ionexd2(isb)+ia+nocc(isblkxder1(1,isb))*(ib                 9d28s16
     $       +nocc(isblkxder1(2,isb))*(idm+nocc(isblkxder1(3,isb))*icm))  9d28s16
              bc(iadj)=bc(iadj)+bc(iadh)                                10d31s16
             end do
            end do
           else                                                          9d20s16
        ...
        do ic=i2s,i2e
         ...
         do id=i10,i1n
          ...
          if(ic.gt.nocc(isblkxder1(4,isb)))then                          9d28s16
           do ib=0,nocc(isblkxder1(2,isb))-1
            do ia=0,nocc(isblkxder1(1,isb))-1
             iadh=ii0+ib+nbasdwsc(isblkxder1(2,isb))*ia
             iadj=ionexd2(isb)+ia+nocc(isblkxder1(1,isb))*(ib
     $       +nocc(isblkxder1(2,isb))*(idm+nocc(isblkxder1(3,isb))*icm))
             bc(iadj)=bc(iadj)+bc(iadh)                                 10d31s16
            end do
...
c
     ...
     $      isblkxder1(3,isb))
       itry3=iptoh(isblkxder1(1,isb),isblkxder1(2,isb),
     $      isblkxder1(4,isb))
       itry4=iptoh(isblkxder1(2,isb),isblkxder1(1,isb),
     $      isblkxder1(4,isb))
       if(itry.gt.0)then                                                9d16s16
     ...
     $      mynprocg,mynowprog,ilh,ihh,i1s,i1e,i2s,i2e)
        ...
        ii0=ihcol(itry)
        if(isblkxder1(3,isb).eq.isblkxder1(4,isb))then
         nrowh=(nbasdwsc(isblkxder1(3,isb))                              9d20s16
     $        *(nbasdwsc(isblkxder1(3,isb))+1))/2                        9d20s16
        else                                                            9d20s16
        ...
        do ic=i2s,i2e
         ...
         do id=i10,i1n
          ...
          if(ic.le.nocc(isblkxder1(2,isb)))then                          9d28s16
           if(isblkxder1(3,isb).eq.isblkxder1(4,isb))then                 9d28s16
            do ib=0,nocc(isblkxder1(3,isb))-1
             do ia=0,nvirtc(isblkxder1(4,isb))-1
              iap=ia+nocc(isblkxder1(4,isb))
              ix=max(iap,ib)+1                                           9d28s16
              in=min(iap,ib)                                             9d28s16
              ix=((ix*(ix-1))/2)+in
              iadh=ii0+ix                                                9d20s16
              iadj=ionexd2(isb)+idm+nocc(isblkxder1(1,isb))*(icm                 9d28s16
     $       +nocc(isblkxder1(2,isb))*(ib+nocc(isblkxder1(3,isb))*ia))  9d28s16
              bc(iadj)=bc(iadj)+bc(iadh)                                10d31s16
             end do
            end do
           else                                                          9d20s16
         ...
         do id=i10,i1n
          ...
          if(ic.le.nocc(isblkxder1(2,isb)))then                          9d28s16
           do ib=0,nocc(isblkxder1(3,isb))-1
            do ia=0,nvirtc(isblkxder1(4,isb))-1
             iap=ia+nocc(isblkxder1(4,isb))
             iadh=ii0+ib+nbasdwsc(isblkxder1(3,isb))*iap
             iadj=ionexd2(isb)+idm+nocc(isblkxder1(1,isb))*(icm
     $       +nocc(isblkxder1(2,isb))*(ib+nocc(isblkxder1(3,isb))*ia))
             bc(iadj)=bc(iadj)+bc(iadh)                                 10d31s16
            end do
     ...
     $       +nocc(isblkxder1(2,isb))*(ib+nocc(isblkxder1(3,isb))*ia))
        ...
        it3=multh(isblkxder1(3,isb),iprop)
        it4=multh(isblkxder1(4,isb),iprop)
        write(6,12)(isblkxder1(j,isb),j=1,2),it3,it4
        call dws_sync
        call dws_finalize
        stop
       end if
       if(idwsdeb.gt.10.and.min(nrow,ncol).gt.0)then
        write(6,*)('ionexd2 ooox at bottom of parajkfromhd2b'),
     $       ionexd2(isb)
         write(6,12)(isblkxder1(j,isb),j=1,4)
         call prntm2(bc(ionexd2(isb)),nrow,ncol,nrow)
         itmp=ibcoff
         ibcoff=itmp+nrow*ncol
         call enough('parajkfromhd2b.  5',bc,ibc)
         do i=0,nrow*ncol-1
