c mpec2.1 version theta. For copyright and Disclaimers, see start.f
     ...
     $     ibc,mden,nstate,isinfo,cdir,cdc,tracer,cdirb,ndentd)         4d10s23
      ...
      integer*8 ibc0,ibcoffo,idndm,idnm,iorbpart,iovrpart,isuma,itmp1
     $itmp2
     $itmp3,itmp5,itriangle1
...
c     dv v
     ...
     $           +bc(iad3)*bc(iad1)                                     3d27s23
       ...
       if(nhj.ne.0)then
        ...
        if(lprt)write(6,*)('what we have for dddd'),dddd                3d14s25
        k=1                                                               3d15s23
     ...
     $        iact(jsb))                                                3d17s23
        ...
        if(lprt)write(6,*)('bodc before dndm*den part '),bodc(1)
        do k=0,mden-1                                                   3d14s23
         kp=k+1                                                         3d14s23
         do i=0,iact(jsb)-1                                              6d20s22
          ip=i+idoub(jsb)                                                6d20s22
        ...
        if(lprt)write(6,*)('bodc after dndm*den part '),bodc(1)
       ...
       if(multh(isblk(1,is),isblk(2,is)).eq.ipuse)then                  7d8s22
        ...
        if(min(nrow,ncol).gt.0)then                                     7d8s22
         if(lprt)then                                                   7d14s22
          ...
          write(6,*)('triangularize this ')                             3d28s25
          ntrir=(iact(isblk(1,is))*(iact(isblk(1,is))+1))/2             3d28s25
          ntric=(iact(isblk(3,is))*(iact(isblk(3,is))+1))/2             3d28s25
          itriangle1=ibcoff                                             3d28s25
          itriangle2=itriangle1+nrow*ntric                              3d28s25
          ibcoff=itriangle2+ntrir*ntric                                 3d28s25
          call enough('bodcpart.triangle1',bc,ibc)                      3d28s25
          do iz=itriangle1,ibcoff-1                                     3d28s25
           bc(iz)=0d0                                                   3d28s25
          end do                                                        3d28s25
          do i4=0,iact(isblk(3,is))-1                                   3d28s25
           do i3=0,iact(isblk(3,is))-1                                  3d28s25
            ix=max(i3,i4)
            in=min(i3,i4)
            itri=((ix*(ix+1))/2)+in
            iad1=iad+nrow*(i3+iact(isblk(3,is))*i4)                     3d28s25
            iad2=itriangle1+nrow*itri                                   3d28s25
            do i12=0,nrow-1                                             3d28s25
             bc(iad2+i12)=bc(iad2+i12)+bc(iad1+i12)                     3d28s25
            end do                                                      3d28s25
           end do                                                       3d28s25
          end do                                                        3d28s25
          write(6,*)('after cols trianglized ')
          call prntm2(bc(itriangle1),nrow,ntric,nrow)
          do i2=0,iact(isblk(2,is))-1
           do i1=0,iact(isblk(2,is))-1
            ix=max(i1,i2)
            in=min(i1,i2)
            itri=((ix*(ix+1))/2)+in
            iad1=itriangle1+i1+iact(isblk(2,is))*i2
            iad2=itriangle2+itri
            do i34=0,ntric-1
          ...
          write(6,*)('after rows and cols trianglized ')
          call prntm2(bc(itriangle2),ntrir,ntric,ntrir)
          ibcoff=itriangle1
         end if                                                         7d14s22
