...
c
     ...
     $     ivoffg(8),ivoffw(8),morbc(*),morb(*),scopy(*),icanog(*)      5d3s23
      ...
      integer*8 ib,ibcoffo,ibct,ibcx,icpy,igeow,imapwg,ioltmp,iortmp
...
c
   ...
   87   continue                                                        4d28s21
     ...
     $         *dfloat(isymg(ixyz,iapairg(2,japairg)))-bc(jqgg+ixyz))**2 4d28s21
          end do                                                         4d28s21
          rms=sqrt(rms/3d0)                                              4d28s21
...
c
     ...
     d' makeguess. 18')
         ...
         do isg=1,nsymbg                                                 10d23s17
          ...
          if(morbp(isg).gt.0)then                                        5d14s19
           isitmp=ibcoff                                                1d23s25
           ibcoff=isitmp+nbasisc(isw)*morbp(isg)*ncomp                  1d23s25
           call enough('makeguess.isitmp',bc,ibc)                       1d23s25
           do i=1,nbasisc(isw)                                          4d27s21
            do j=1,morbp(isg)*ncomp                                     10d7s22
             iad1=isitmp+i-1+nbasisc(isw)*(j-1)                         1d23s25
     ...
     $        bc(ixi),nbasisc(isw),bc(isitmp),nbasisc(isw),0d0,         1d23s25
     $        bc(itemp),nbasisc(isw),                                   4d27s21
     d' makeguess. 19')
           ibcoff=isitmp                                                1d23s25
           if(ldebug)then
     ...
     $        nbasdwsg(isg))                                            10d26s17
           ...
           do i=0,nbasdwsg(isg)-1
            ...
            if(bc(iad).gt.0.25d0)then                                   1d24s25
     ...
     $          (' vectors ')
            ...
            do i=0,nkeep-1                                                10d26s17
             ...
             if(ldebug)write(6,*)('what remains of vector '),ip,        1d24s25
     $            (' is '),sum                                          1d24s25
             if(sum.gt.1d-10)then                                       1d24s25
 ...
 3357      format(5i5)
     ...
     $       nbasisc(isw))                                              4d28s21
          call dgemm('t','n',nbasisc(isw),nbasisc(isw),nbasisc(isw),1d0,
     $      vecr(ivecsav0),nbasisc(isw),vecr(ivecsav0),nbasisc(isw),0d0,
     $         bc(ibcoff),nbasisc(isw))
          write(6,*)('ortho test ')
          call prntm2(bc(ibcoff),nbasisc(isw),nbasisc(isw),nbasisc(isw))
          write(6,*)('myguessi = '),myguessi
         end if                                                         7d5s23
         if(myguessi.ne.2)then                                           4d28s21
          write(6,*)('previously commented out block ')
          write(6,*)('instead set myguessi to 2 ')
          myguessi=2                                                    1d27s25
         end if                                                         4d28s21
         ibcoff=ixi                                                      10d24s17
        end if                                                          5d14s19
...
c
     ...
     d' makeguess. 30')
        ...
        if(ldebug)then                                                  5d3s21
         write(6,*)('orthogonalized vectors '),ioffo
         call prntm2(vecr(ioffo+1),nrow,nbasdws(isb),nrow)               2d15s19
         if(myguessi.eq.1)then                                           1d23s25
          itmpot=ibcoff                                                   1d23s25
          ibcoff=itmpot+nrow*nbasdws(isb)                                1d23s25
          call enough('makeguess.tmpot',bc,ibc)                         1d23s25
          call dgemm('n','n',nrow,nbasdws(isb),nrow,1d0,bc(iovr),        1d23s25
     $        nrow,vecr(ioffo+1),nrow,0d0,bc(itmpot),nrow)              1d23s25
          call dgemm('t','n',nbasdws(isb),nbasdws(isb),nrow,1d0,         1d23s25
     $        vecr(ioffo+1),nrow,bc(itmpot),nrow,0d0,bc(ibcoff),        1d23s25
     $        nbasdws(isb))                                             1d23s25
         else                                                            1d23s25
          call dgemm('t','n',nbasdws(isb),nbasdws(isb),nrow,1d0,
     $       vecr(ioffo+1),nrow,vecr(ioffo+1),nrow,0d0,
     $       bc(ibcoff),nbasdws(isb))
         ...
         otest=0d0                                                       1d23s25
         do i=0,nbasdws(isb)-1                                          1d23s25
          do j=0,i-1                                                    1d23s25
           ji=ibcoff+j+nbasdws(isb)*i                                   1d23s25
           ij=ibcoff+i+nbasdws(isb)*j                                   1d23s25
           otest=otest+bc(ji)**2+bc(ij)**2                              1d23s25
          end do                                                        1d23s25
          ii=ibcoff+i+nbasdws(isb)*i                                    1d23s25
          otest=otest+(bc(ii)-1d0)**2                                   1d23s25
         end do                                                         1d23s25
         otest=sqrt(otest)/dfloat(nbasdws(isb))                         1d23s25
         write(6,*)('orthogonality test '),otest                        1d23s25
         if(otest.gt.1d-7)then                                          1d23s25
          call prntm2(bc(ibcoff),nbasdws(isb),nbasdws(isb),nbasdws(isb))1d23s25
          stop 'makeguess'                                              1d23s25
         end if                                                         1d23s25
        end if
