...
c
     ...
     $     mynnode                                                      5d24s18
      ...
      integer*8 iafgencsf3,iamconverged,ib4,ibasisc,ibasisf,ibc1000
     $ibcb4
     $ibcb4i,ibcoffo,ibufr,ibufvcvds,icol,icsf,icsf2,icsfpd
     $idavide,idddig,iddoub,idenput,idest,idorb,idorbfi,idvi,idvmt
     $ieigd,ieigintxyz,ieigvspace,ieold,iepart,iewgt,iff,ifhis
     $igss,ih0a,ih0ae,ih0copy,ihddiag,ihdi,ihsdiag,ihxy,ilastde
     $imsg,intvec,iorg,ipairs,ipassr,iptrbit,iptrcb,iptrfbit
     $iptrfibit,issdig,isymo,itimer,ivdinout,ividotvi,ivint
     $ivintlast,ivints,ivintt,ivstry,ivtmp,ivvn,jdenpt,jptrf
     $nfcnpx,nfcnpxfi,nff0,nff1,nff2,nff2x,nffr,nlastde
     ...
     $     nrestart,iftype,fstgth,ndfld,lambdaci,bc,ibc,idarot)         12d6s23
      if(iprtr(31).ne.0.and.mynowprog.eq.0)then                         12d4s24
       write(6,*)('input orbitals: ')                                   12d4s24
       do isb=1,nsymb                                                   12d4s24
        if(nbasdws(isb).gt.0)then                                       12d4s24
         write(6,*)('for symmetry block '),isb                          12d4s24
         call prntm2(bc(iorb(isb)),nbasisp(isb),nbasdws(isb),           12d4s24
     $        nbasisp(isb))                                             12d4s24
        end if                                                          12d4s24
       end do                                                           12d4s24
      end if                                                            12d4s24
...
c
     ...
     $     idum,bc,ibc)                                                 5d20s24
      ...
      ibcoff=imsg+mynprocg                                              ECG_052925
      iter1=0
      idwsdeb=0
...
c
     ...
     $       (bc(ieigint+i)+shift,i=0,nroot-1)                          5d27s21
        ...
        if(nwavef.ne.0)then                                             3d23s21
         ...
         nwavrec=nwavrec+1
         write(2)(bc(ieigint+i)+shift,i=0,nroot-1),                     12d9s24
     $           (bc(ieigint+i)+shift,i=0,nroot-1)                      12d9s24
 ...
 3338   format('rather then using internal ci vectors to generate',
  ...
  239   format(4x,'#',9x,'sym',x,'roots',2x,'nlzz',4x,'L',4x,'Lz')      8d24s21
   ...
   65   format(i1)                                                      4d25s21
     ...
     $       toldv,maxdiisci,maxrestci,iusenormalci)                    1d28s25
...
c                                                                       6d17s21
     ...
     $       ibc(idenput),idenorg,ldebugf)                              3d26s25
       else                                                             6d7s21
 ...
 1000  continue                                                         11d13s18
  ...
  221     format(/18x,'mrci-s')                                          9d5s23
          ...
          if(iunc(2).eq.0)then                                           9d5s23
           write(6,222)                                                  9d5s23
  222      format(/15x,'i2cmrci',7x,'i2cmrci+q(h,RD)',5x,               11d22s24
     $        'ref',13x,'ecorr',8x,'Vref*Vi2cmrci',3x,                  11d22s24
     $        'Fraci Vi2cmrci',2x,'Hin i',11x,'Hin is',10x,              11d22s24
     $          'Fracsi Vi2cmrci')                                      11d22s24
          else                                                           9d5s23
           write(6,22)                                                     10d1s19
   22      format(/18x,'mrci',12x,'mrci+q(h,RD)',                       11d22s24
     $        4x,'ref',13x,'ecorr',11x,'Vref*Vmrci',6x,                 10d1s19
     $        'Fraci Vmrci',5x,'Hin i',11x,'Hin is',10x,'Fracsi Vmrci') 11d22s24
          end if                                                          9d5s23
         end if                                                         9d6s23
        end if                                                          1d18s20
        ...
        if(lprint)then                                                  7d21s21
         idavide=ibcoff                                                 11d29s24
         ibcoff=idavide+nroot                                           11d29s24
         call enough('mrci.davide',bc,ibc)                              11d29s24
     ...
     $        ibc(icsf),dot1)                                           11d15s21
          ...
          ecorrr=bc(ieigint+jm)-dotq                                    11d22s24
          davidrr=dotq+ecorrr/dot2                                      11d22s24
          davidq=bc(ieigintref+jm)+ecorr/(dot1**2)                       3d18s19
          davidr=bc(ieigintref+jm)+ecorr/(dot2)                          3d18s19
          davidhyb=0.5d0*(davidsx+davidrr+shift)                        11d22s24
          bc(idavide+jm)=davidhyb                                       11d29s24
          write(6,1676)spinm,termsym,                                    1d5s20
     $        j,bc(ieigint+jm)+shift,davidhyb,                          11d22s24
     $        bc(ieigintref+jm)+shift,                                  2d9s21
     $        ecorr,dot1,dot2,dotq+shift,dotqs+shift,dot2s,davidrr+shift11d22s24
 1676    format(a2,a6,'>',i3,13f16.10)                                  11d22s24
        else
          write(6,1676)spinm,termsym,                                    1d5s20
 ...
 1525    format(100000a1)                                               11d21s22
        ...
        if(mynowprog.eq.0.and.nwavef.ne.0)then                          3d23s21
         ...
         write(6,*)('idavide = '),idavide
         write(2)(bc(ieigint+i)+shift,i=0,nroot-1),(bc(idavide+i),      11d29s24
     $        i=0,nroot-1)                                              11d29s24
         ipack2(1)=nlzzci                                               10d7s24
         ipack2(2)=nder                                                 10d7s24
         write(2)ipack4(1),lambdaci                                     10d7s24
         if(nsing.ne.0)then
     ...
     $       idenhvv,idenj,idenk,bc,ibc,ibc(idenput),idenorg,ldebugf)   3d26s25
        i18=1                                                            1d8s21
