c mpec2.1 version theta. For copyright and Disclaimers, see start.f
      subroutine paraeri(natom,ngaus,ibdat,nbasis,ihmat,iorb,nocc,ipair,5d12s10
     $     nhcolt,isym,iapair,ibstor,isstor,multh,iptoh,iter,idwsdeb,   8d24s15
     $     idorel,ascale,nbasisp,ifull4x,icode,i3xd,itt4v,iaddr,naddr,  5d20s24
     $     bc,ibc)                                                      5d20s24
c
c     for HF, CAS nbasdwsd(i)=nbasdws(i)*ncomp [=nbasdwsc(i) in parahf] 5d29s18
c     for mrci, nbasdwsd(i)=nbasdws(i), because we will not excite into  5d29s18
c     positron states.                                                  5d29s18
c
c     compute (rs|cd] with rs in ao basis and cd in mo basis.
c     ifull4x=1: cd are noc only.
c     ifull4x=2: c all, d noc only
c     ifull4x=3: cd both all.
c
      ...
      integer*8 i12,iabstrt,ibcoffo,ieraw,ieraw2,ihmat,ipair,ipta,iptap,iptra,isnd,itmp,itmp1,itmpab,itmpab2
     ...
     $     mynnode
      ...
      write(6,*)('in paraeri '),ibcoff,iter,idorel
...
c
 ...
 3315  format(i5,5x,4i2,5x,i8)                                          5d12s10
      ...
      ndoit=0
      nxtot=0
      if(icode0.ne.0)then                                                4d2s24
       isnd=ibcoff                                                      4d15s24
       ircv=isnd+mynprocg                                               4d15s24
       ipt=ircv+mynprocg                                                4d15s24
       ipr=ipt+mynprocg                                                 4d15s24
       ibcoff=ipr+mynprocg                                              4d15s24
       call enough('paraeri.sndrcv',bc,ibc)                             4d15s24
       call dorb4v(idum,nsymb,multh,nbasisp,idum,nerimat,ncomp,iorb,    4d4s24
     $     ibc(i3xd),ibc(i3xd+1),ibc(i3xd+2),ibc(i3xd+3),ipair,nn,      4d11s24
     $     ngaus,idum,idum,idum,idum,idum,                              4d11s24
     $     isstor,ibstor,iptap,iptbp,idum,idum,srh,0,jbdat,nvirt,nduse, 4d12s24
     $     nbasdws,iapair,ibc(isnd),ibc(ircv),ibc(ipt),ibc(ipr),bc,ibc) 4d15s24
      end if
      igoal2=3
 ...
 3352  format(6i5)
       ...
       call enough('paraeri.pta',bc,ibc)                                4d9s24
       jpta=ipta-1                                                      4d9s24
       jptb=iptb-1                                                      4d9s24
     ...
     $          ntmpmat                                                 4d9s24
       ...
       call enough('paraeri.tmpab2',bc,ibc)                             4d9s24
     ...
     $          nerimat                                                 4d9s24
       ...
       do isd=1,nsymb                                                   5d12s10
        do isc=1,nsymb                                                  5d12s10
         ...
         ibcoff=itmpab(isc,isd)+ncsza0*nduse2(isc)*nduse(isd)*ntmpmat   8d18s23
        end do                                                          5d12s10
       end do                                                           5d12s10
       if(icode.ne.0)then                                               3d4s24
        iptra=ibcoff                                                    3d4s24
        iptrb=iptra+nszaa                                               3d4s24
        ibcoff=iptrb+nszba                                              7d16s24
       end if                                                           3d4s24
       memneed(1:5)='tmpab'
       call enough('paraeri.  3',bc,ibc)
       ...
       do isd=1,nsymb                                                   5d12s10
        do isc=1,nsymb                                                  5d12s10
         ...
         ibcoff=ieraw(isc,isd)+ncsz0*nbasisp(isc)*nbasisp(isd)*nerimat  10d28s20
        end do                                                          5d12s10
       end do                                                           5d12s10
       memneed(1:4)='eraw'
       call enough('paraeri.  4',bc,ibc)
       ...
       do j=1,nn                                                        3d4s10
        ...
        call enough('paraeri.tmpn',bc,ibc)                              4d9s24
     ...
     $       ieri,jdoit,1d0,idorel,ascale,nmat,bc,ibc)                  11d9s22
             ...
             call erir(ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3), 4d9s24
     ...
     $           +nbn(isb)*(iddd+nbasisp(isd)*(iccc+nbasisp(isc)*in)))  4d10s24
       ...
       do isd=1,nsymb                                                   4d9s24
        ...
        if(icode.eq.0)then                                              4d11s24
         ntest=nduse(isd)                                               4d11s24
        else                                                            4d11s24
         ntest=nbasdws(isd)                                             4d11s24
        end if                                                          4d11s24
        do isc=1,nsymb                                                  4d9s24
         if(icode.eq.0)then                                             4d11s24
          ntestc=nduse2(isc)                                            4d11s24
         else                                                           4d11s24
          ntestc=nbasisp(isc)                                           4d11s24
         end if                                                         4d11s24
         iscd=multh(isc,isd)                                            4d9s24
         nbpc=nbasisp(isc)*ncomp                                         4d9s24
         do isb=1,nsymb                                                 4d9s24
          isa=multh(isb,iscd)                                           4d9s24
          if(min(nan(isa),nbn(isb),ntestc,ntest).gt.0)then              4d11s24
           nab=nan(isa)*nbn(isb)                                        4d9s24
           nabc=nab*nbasisp(isc)                                        4d9s24
           nabd=nab*nduse(isd)                                          4d9s24
           nabcd=nabc*nbasisp(isd)                                      4d9s24
           itmp1=ibcoff                                                 4d9s24
           if(icode.eq.0)then                                           4d10s24
            itmp2=itmp1+nabc*nduse(isd)                                  4d9s24
           else                                                         4d10s24
            itmp2=itmp1+nabc*nbasdws(isd)                                  4d9s24
           end if                                                       4d10s24
           ibcoff=itmp2+nabd*nduse2(isc)                                4d9s24
           call enough('paraeri.tmp12',bc,ibc)                          4d9s24
           ...
           do in=0,nerimat-1                                            4d9s24
            ...
            if(icode0.eq.0.and.itt4v.eq.0)then                          6d5s24
             if(nduse(isd).gt.0)then                                    6d20s24
              call dgemm('n','n',nabc,nduse(isd),nbasisp(isd),1d0,        4d9s24
     $           bc(jeraw2),nabc,bc(jorbd),nbpd,0d0,bc(itmp1),nabc,     4d9s24
     $           'paraeri.eraw2')                                       4d9s24
             end if                                                     6d20s24
            else                                                        4d10s24
             call dgemm('n','n',nabc,nbasdws(isd),nbasisp(isd),1d0,     4d10s24
     $           bc(jeraw2),nabc,bc(jorbd),nbpd,0d0,bc(itmp1),nabc,     4d9s24
     $           'paraeri.eraw2')                                       4d9s24
             if(icode0.ne.0)then                                        6d5s24
              innpass=in
              call dorb4v(itmp1,nsymb,multh,nbasisp,nab,nerimat,ncomp,    2d29s24
     $        iorb,ibc(i3xd),ibc(i3xd+1),ibc(i3xd+2),ibc(i3xd+3),isa,   4d10s24
     $        -1,isb,isc,ja4,nszaa0,jb4,nszba0,isstor,ibstor,           4d10s24
     $        iptap,iptbp,nan(isa),nbn(isb),srh,0,innpass,nvirt,        4d12s24
     $            nduse,                                                4d11s24
     $        nbasdws,i,idum,idum,idum,idum,bc,ibc)                     4d15s24
             end if                                                     6d5s24
             if(itt4v.ne.0)then                                          5d20s24
              call tt4v(itt4v,iaddr,naddr,bc(itmp1),nsymb,multh,nbasisp,6d5s24
     $           nerimat,isa,isb,isc,isd,nan,iptap,nbn,iptbp,in,        5d31s24
     $           ncomp,lsame,nocc,nvirt,bc,ibc)                         6d5s24
             end if                                                     6d5s24
            end if                                                      4d10s24
     ...
     $           'paraeri.eraw2c')                                      4d9s24
         ...
         if(icode0.ne.0)then                                             4d11s24
          call dorb4v(itmp1,nsymb,multh,nbasisp,nab,nerimat,ncomp,      4d12s24
     $        iorb,ibc(i3xd),ibc(i3xd+1),ibc(i3xd+2),ibc(i3xd+3),isa,   4d10s24
     $        -2,isc,isd,ja4,nszaa0,jb4,nszba0,isstor,ibstor,           4d12s24
     $        iptap,iptbp,nan(isa),idum,srh,0,in,nvirt,                 8d7s24
     $            nduse,                                                4d11s24
     $        nbasdws,iapair,idum,idum,idum,idum,bc,ibc)                                    4d12s24
         end if                                                         4d11s24
        end do                                                          4d9s24
       end do                                                           4d9s24
c
...
c
     ...
     $              +nan(isa)*(ibbb+nbn(isb)*in))                         4d9s24
      ...
      if(icode0.ne.0)then                                                4d2s24
          call dorb4v(ieraw,nsymb,multh,nbasisp,ncsz0,nerimat,ncomp,    2d29s24
     $        iorb,ibc(i3xd),ibc(i3xd+1),ibc(i3xd+2),ibc(i3xd+3),ipassa,4d5s24
     $        npassa,                                                   4d5s24
     $        ipassb,npassb,ja4,nszaa0,jb4,nszba0,isstor,ibstor,        4d2s24
     $        ibc(iptra),ibc(iptrb),nsza0,nszb0,srh,1,nocc,nvirt,nduse, 4d2s24
     $        nbasdws,iapair,idum,idum,idum,idum,bc,ibc)                4d15s24
         end if                                                         2d29s24
      return
      end                                                               2d19s10
