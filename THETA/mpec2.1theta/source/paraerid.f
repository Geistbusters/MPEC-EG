c mpec2.1 version theta. For copyright and Disclaimers, see start.f
      subroutine paraerid(natom,ngaus,ibdat,nbasis,ihmat,iorb,nocc,     3d2s16
     $     ipairab,nhcolt,isym,iapair,ibstor,isstor,multh,iptoh,iter,   3d2s16
     $     idwsdeb,idorel,ascale,iatom,ixyz,ipropsym,idersign,iordr,    4d7s22
     $     nbasisp,bc,ibc)                                              11d9s22
c
c     like paraeri, except form info for derivative of (ab|cd)
...
c
     ...
     $     multh(8,8),iptoh(8,8,8),nocc(8),neraw(8,8),nueraw(8,8),      4d7s22
     $     nbasisp(*)                                                   4d7s22
     ...
     $     mynnode
      ...
      data icall/0/
      save icall
      integer*8 i12,ibcexit,ibcoffo,ieraw,ihalf,ihmat,ipairab,iszz,itmp
     $itmpab
      icall=icall+1
      if(idwsdeb.gt.10)then
       write(6,*)('in paraerid !!!!! '),natom,ngaus,iter,idwsdeb,
     $     idorel,ascale,iatom,ixyz
       write(6,*)('idersign = '),idersign
       write(6,*)('for call no. '),icall
      end if                                                            5d19s22
...
c
      ...
      do i=1+mynowprog,nnab,mynprocg                                      3d18s10
       ...
       do ib=1,nba0                                                     9d18s15
        ...
        do ia=1,naa0                                                    9d18s15
         ...
         isad=multh(isa,ipropsym)                                       2d23s16
         isab=multh(isad,isb)                                           2d23s16
         do isd=1,nsymb                                                 5d12s10
          isc=multh(isd,isab)                                           5d12s10
          nadd=nbasdws(isc)*nocc(isd)                                   5d31s22
      ...
      nhcolt=0                                                          5d12s10
      do i=1,nsdlkd                                                     5d12s10
       if(iter.eq.1)write(6,3315)i,(isblkd(j,i),j=1,4),nhcol(i)         3d1s16
 3315  format(i5,5x,4i2,5x,i8)                                          5d12s10
     ...
     $     (' elements of hmat')                                        3d15s12
      ...
      memneed(1:4)='hmat'
      call enough('paraerid.  2',bc,ibc)
      ihcol(1)=ihmat                                                    5d12s10
      do i=2,nsdlkd                                                     3d1s16
 ...
 3352  format(6i5)
       ...
       do isd=1,nsymb                                                   5d12s10
        do isc=1,nsymb                                                  5d12s10
         ...
         ibcoff=itmpab(isc,isd)+ncsza0*nbasdws(isc)*nocc(isd)*ntmpmat   5d31s22
        end do                                                          5d12s10
       end do                                                           5d12s10
       memneed(1:5)='tmpab'
       call enough('paraerid.  4',bc,ibc)
       ...
       do ipassb=1,nszba/nszb                                           5d12s10
        ...
        do ipassa=1,nszaa/nsza                                          5d12s10
         ...
         if(iordr.eq.2)siga=1d0                                         4d27s22
c
c     note: integral routines return derivative wrt the electron        5d19s22
c     coordinate, which is the negative of the derivative wrt the       5d19s22
c     nuclear coordinate, which is what we want.                        5d19s22
         if(iordr.eq.1)siga=-drsign*siga                                8d20s24
         ioab=iob+nszba0*ioa                                            5d31s22
         if(idoit.eq.1)then
     ...
     $             ieri,0,siga,idx,idy,idz,0,0,0,0,0,0,0,0,0,bc,ibc)    11d14s22
            else                                                        5d31s22
             call restind                                               5d31s22
     ...
     $       ieri,jdoit,siga,idorel,ascale,nmat,idx,idy,idz, 0,0,0,     5d31s22
     $            0,0,0, 0,0,0,bc,ibc)                                  11d10s22
            end if                                                      5d31s22
            jtmp=itmp                                                     10d28s20
            jeri=ieri                                                     10d28s20
           ...
           if(iapair(1,ibc(jc8)).gt.0)then                                5d12s10
            ...
            if(ibc(ja8).eq.iatom)then                                   4d18s16
             if(idorel.eq.0)then                                         5d31s22
              call erird(ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3)
     $             ,ibc(jb),bc(jb2),bc(jb3),cartb,cartb(2),cartb(3),
     ...
     $            0,0,0, 0,0,0,bc,ibc)                                  11d10s22
            ...
            if(iapair(1,ibc(jc8)).gt.0)then                                5d12s10
             if(ibc(ja8).eq.iatom)then                                   4d18s16
              if(idorel.eq.0)then                                         5d31s22
               call erird
     $             (ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3)
     $             ,ibc(jb),bc(jb2),bc(jb3),cartb,cartb(2),cartb(3),
     $             ibc(jc),bc(jc2),bc(jc3),cartc,cartc(2),cartc(3),     5d31s22
     $             ibc(jd),bc(jd2),bc(jd3),cartd,cartd(2),cartd(3),     5d31s22
     $             ieri,0,siga,idx,idy,idz,0,0,0,0,0,0,0,0,0,bc,ibc)    1d10s23
              else                                                        5d31s22
               call restind                                               5d31s22
     $            (ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3),     5d31s22
     $           ibc(jb),bc(jb2),bc(jb3),cartb,cartb(2),cartb(3),       3d13s20
...
c                                                                       1d10s11
         do isd=1,nsymb                                                  5d12s10
          if(nocc(isd).gt.0)then                                        6d2s10
           do isc=1,nsymb                                                 5d12s10
            if(nbasdws(isc).gt.0)then                                   6d2s10
             nabc=ncsz0*nbasisp(isc)                                    5d31s22
             ihalf=ibcoff                                                  5d12s10
             ibcoff=ihalf+nabc*nocc(isd)                                   5d12s10
             memneed(1:5)='ihalf'
             call enough('paraerid.  7',bc,ibc)
             if(idoit.eq.1)then
              write(6,*)('vectors: '),isc,isd
              call prntm2(bc(iorb(isd)),nbasis(isd)*ncomp,nocc(isd),    4d7s22
     $             nbasis(isd)*ncomp)                                   4d7s22
              write(6,*)('integrals ')
              nnz=0d0
              do ixd=1,nabc*nbasisp(isd)*nerimat                        5d31s22
               if(abs(bc(ieraw(isc,isd)+ixd-1)).gt.1d-6)nnz=nnz+1
              end do
...
c
       ...
       if(iapair(1,ibc(ja8)).gt.0)then                                  5d12s10
        do isd=1,nsymb                                                  5d12s10
         do isc=1,nsymb                                                 5d12s10
          ncdb=nbasdws(isc)*nocc(isd)*nszba0                            5d31s22
          jtmpab=itmpab(isc,isd)                                        10d28s20
...
c
      ...
      do isb=1,nsymb                                                    1d10s11
       do isc=1,nsymb                                                   1d10s11
        isbc=multh(isc,isb)                                             4d18s16
        do isd=1,nsymb                                                  1d10s11
         isad=multh(isd,isbc)                                           4d18s16
         isa=multh(isad,ipropsym)                                       4d18s16
         ii=iptoh(isd,isc,isb)                                          1d10s11
         ncd=nbasdws(isc)*nocc(isd)                                     4d7s22
         if(ii.gt.0.and.ncd.gt.0)then
          if(nhcol(ii).gt.0)then                                        1d10s11
           write(6,*)('integrals for symmetry '),isad,isb,isc,isd,
     $         (' (or '),isa,isb,isc,isd,(')')                          4d18s16
