...
c
     ...
     $     iopso(5,33),ixmtso(8,id),iden(8),ident(8),iorbo(8)           2d3s25
     ...
     $     mynnode
      ...
      integer*8 ibcb4,ibcmaxbx,ibcoff0,ibcoffo,ibctop,ibctopi,ibodcp
     ...
     $      (' wave function matrix elements for you.')
      ...
      noff=0                                                            5d12s21
c
c     iprop(6) ne 0 for diabatic states
c     iprop(7) ne 0 for natural orbitals
c
      if(natom.ne.1.and.iprop(6).eq.0.and.iprop(7).eq.0.and.            2d3s25
     $     ibodc.eq.0)then                                              2d3s25
...
c                                                                       6d17s21
     ...
     $      maxbxd,1,bc,ibc,mddilow,mddihig,itargs,itargd,iqhrc)        12d9s24
     ...
     $       elowest,idorel,nsymb,nbasp,nbasdws,nvirt,multh,maxbx,      7d21s21
     $      maxbxd,1,bc,ibc,nddilow,nddihig,itargs,itargd,iqhrc)        12d9s24
         maxbxi=maxbx+npadddi                                            7d30s22
         maxbxdi=maxbxd+npadddi                                          7d30s22
         call dws_synca                                                    7d9s21
     ...
     $        idum,idum,dum,1,idum,idum,1,dum,lprint,l2sub,bc,ibc,n4vso)2d8s23
      ...
      if(ibodc.ne.0)then                                                2d3s25
       write(6,*)('computing bodc via finite difference')               2d3s25
       if(idorel.eq.0)then                                               1d11s20
        ncomp=1                                                          1d11s20
       else                                                              1d11s20
        ncomp=2                                                          1d11s20
       ...
       iorbf=ibcoff                                                     2d3s25
       ibdatf=iorbf+nfname*nsymb                                        2d3s25
       ipdat=ibdatf+nfname                                              2d3s25
       ibcoff=ipdat+2                                                   2d3s25
       jbdatf=ibdatf-1                                                  2d12s25
       call enough('prop.ipdat',bc,ibc)                                 2d3s25
       if(mynowprog.eq.0)then                                           2d3s25
        call prebodcfd(ifname,nfname,natom,nuniqs,idatal,ndatal,        2d3s25
     $      ibc(ibdatf),nbdat,bc,ibc)                                   2d3s25
        write(6,*)('nuniqs,ndatal from prebodcfd '),nuniqs,ndatal
        write(6,*)('ibcoff after prebodcfd '),ibcoff
        ibc(ipdat)=nuniqs                                               2d3s25
        ibc(ipdat+1)=ndatal                                             2d3s25
        write(6,*)('ipdat: '),ibc(ipdat),ibc(ipdat+1)
       end if                                                           2d3s25
       itestr=1715564+9*3+1
       write(6,*)('what''s under '),itestr,bc(itestr),ibcoff
       xbdat=dfloat(nbdat)                                              2d13s25
       call dws_bcast(xbdat,1)                                          2d13s25
       nbdat=nint(xbdat)                                                2d13s25
        write(6,*)('ipdat: '),ibc(ipdat),ibc(ipdat+1)
       call bodcfdb(ibc(ibdatf),nbdat,nfname,bc,ibc)                    2d3s25
       write(6,*)('ibdatf: '),(ibc(jbdatf+idws),idws=1,nfname)
       write(6,*)('ipdat '),ipdat
        write(6,*)('ipdat: '),ibc(ipdat),ibc(ipdat+1)
       write(6,*)('what''s under '),itestr,bc(itestr),ibcoff
       call dws_bcast(bc(ipdat),2)                                      2d3s25
       nuniqs=ibc(ipdat)                                                2d13s25
       ndatal=ibc(ipdat+1)                                              2d13s25
       write(6,*)('nuniqs,ndatal '),nuniqs,ndatal
       write(6,*)('what''s under '),itestr,bc(itestr),ibcoff
       if(mynowprog.ne.0)then                                           2d3s25
        idatal=ibcoff                                                   2d3s25
        ibcoff=idatal+ndatal                                            2d3s25
        call enough('prop.idatal',bc,ibc)                               2d3s25
       end if                                                           2d3s25
       call dws_bcast(bc(idatal),ndatal)                                2d3s25
       write(6,*)('what''s under '),itestr,bc(itestr),ibcoff
       write(6,*)('ibdatf: '),(ibc(jbdatf+idws),idws=1,nfname)
       if(mynowprog.eq.0)then                                           2d3s25
        jfname=ifname                                                   2d3s25
        ifnp=ibcoff                                                      2d3s25
        ibcoff=ifnp+nfname                                               2d3s25
        call enough('prop.fnp',bc,ibc)                                   2d3s25
        ibc(ifnp)=jfname                                                2d3s25
        jfnp=ifnp-1                                                     2d3s25
        nhere=ibc(jfname)                                               2d3s25
        jfname=jfname+nhere+1                                           2d3s25
        do i=2,nfname                                                   2d3s25
         ibc(jfnp+i)=jfname                                             2d3s25
         nhere=ibc(jfname)                                              2d3s25
         jfname=jfname+nhere+1                                          2d3s25
        end do                                                          2d3s25
       end if                                                           2d3s25
       ...
       iout=ibcoff                                                      11d24s19
       write(6,*)('ibcoff before getwf'),ibcoff
       call getwf(fname(1:nhere),ismult,isymmrci,nroot,iout,            2d11s22
     $       iwavedat(1,iwnxt),mdoox,mxopn,iorb,1,noff,nspc,nwavedat,   7d27s22
     $       elowest,idorel,nsymb,nbasp,nbasdws,nvirt,multh,maxbx,      7d21s21
     $      maxbxd,1,bc,ibc,nddilow,nddihig,itargs,itargd,iqhrc)        12d9s24
       write(6,*)('iorb after getwf '),(iorb(isb),isb=1,nsymb)
       write(6,*)('what''s under '),itestr,bc(itestr)
       write(6,*)('ibdatf: '),(ibc(jbdatf+idws),idws=1,nfname)
       nec=iwavedat(7,iwnxt)                                            2d13s25
       write(6,*)('setting nec to '),nec
       write(6,*)('iprop(3,4): '),iprop(3),iprop(4)
       mxopn=iprop(3)-2*iprop(4)                                        2d13s25
       mxopn=max(mxopn,2)                                               2d13s25
       write(6,*)('mxopn '),mxopn
       mdon=(nec-mxopn)/2                                                12d17s19
       write(6,*)('settingc mdon to '),mdon,nec,mxopn
       write(6,*)('what''s under '),itestr,bc(itestr)
       jorbf=iorbf-1                                                    2d3s25
       do isb=1,nsymb                                                   2d3s25
        ibc(jorbf+isb)=iorb(isb)                                        2d3s25
       end do                                                           2d3s25
       nspin=(iprop(2)+2-iprop(1))/2                                     7d26s22
       write(6,*)('nspin = '),nspin
       call stepwisecsf(mxopn,spin,spin,dum,idum,idum,0,idum,bc,ibc)    2d13s25
       write(6,*)('what''s under '),itestr,bc(itestr)
       write(6,*)('ibdatf: '),(ibc(jbdatf+idws),idws=1,nfname)
       mdoou=(nec-ispin+1)/2                                            2d24s21
       icsfid=mdoou+1-mdon                                              2d13s25
       write(6,*)('computing icsfid from mdoou,mdon '),mdoou,mdon       2d13s25
       icsfpd=ibcoff                                                     6d11s19
       ibcoff=icsfpd+mdoou+1-mdon                                       2d13s25
       write(6,*)('space for icsfpd '),ibcoff+1-icsfpd
       icsf2=ibcoff                                                      8d2s19
       icsf=icsf2+2*icsfid*nspin                                         7d27s22
       write(6,*)('space for icsf2 '),icsfid,nspin,icsf+1-icsf2
       write(6,*)('loc '),loc(ibc(icsf2)),loc(ibc(icsf))
       ibcoff=icsf+icsfid*nspin                                          7d27s22
       write(6,*)('what''s under '),itestr,bc(itestr)
       call enough('prop.  2',bc,ibc)
       do i=icsf,ibcoff-1                                                10d26s20
     ...
     $      bc,ibc)                                                     11d11s22
       write(6,*)('back from gencsf3 ')
       write(6,*)('what''s under '),itestr,bc(itestr)
       write(6,*)('ibdatf: '),(ibc(jbdatf+idws),idws=1,nfname)
       idovr=ibcoff                                                      2d11s22
       ibcoff=idovr+nroot*nroot                                         2d3s25
       ibcmaxbx=ibcoff
       ibcoff=ibcmaxbx+1
       ipack4(1)=maxbx
       ibc(ibcmaxbx)=ipack8
       ...
       noderw=0                                                         2d3s25
       do isb=1,nsymb                                                   2d3s25
        noderw=noderw+nbasdws(isb)*nbasdws(isb)                         2d3s25
        iden(isb)=ibcoff                                                2d27s25
        ibcoff=iden(isb)+nbasdws(isb)*nbasdws(isb)                      2d27s25
        nh0(isb)=nbasdws(isb)-idoubo(isb)                               2d27s25
       end do                                                           2d3s25
       do isb=1,nsymb                                                   3d14s25
        ident(isb)=ibcoff                                               3d14s25
        ibcoff=ident(isb)+nbasdws(isb)*nbasdws(isb)                     3d14s25
       end do                                                           3d14s25
       nvderw=nroot*nroot                                               2d3s25
       ioder1=ibcoff                                                    2d3s25
       ioder2=ioder1+noderw*nuniqs                                      2d3s25
       ivder1=ioder2+noderw*nuniqs                                      2d3s25
       ivder2=ivder1+nvderw*nuniqs                                      2d3s25
       idder1=ivder2+nvderw*nuniqs                                      2d27s25
       idder2=idder1+noderw*nuniqs                                      2d27s25
       ibcoff=idder2+noderw*nuniqs                                      2d27s25
       call enough('prop.oder12',bc,ibc)                                2d3s25
       do iz=ioder1,ibcoff-1                                            2d3s25
        bc(iz)=0d0                                                      2d3s25
       end do                                                           2d3s25
       do iz=0,noderw-1                                                 3d14s25
        bc(ident(1)+iz)=0d0                                             3d14s25
       end do                                                           3d14s25
       call psipsiden(iwavedat(1,1),nroot,ident,iwavedat(1,1),mdon,     3d14s25
     $        mdooup,ixw1,ixw2,ism,irel,irefo,norb,nbasdws,idoubo,nvirt,3d10s25
     $        maxbx,maxbxd,srh,sr2,multh,nsymb,bc(icsf),bc,ibc)         3d10s25
       call dws_gsumf(bc(ident(1)),noderw)                              3d14s25
       write(6,*)('density for reference wf')
       do isb=1,nsymb                                                   3d14s25
        call prntm2(bc(ident(isb)),nbasdws(isb),nbasdws(isb),           3d14s25
     $       nbasdws(isb))                                              3d14s25
       end do                                                           3d14s25
       jdatal=idatal                                                    2d3s25
       write(6,*)('what''s under '),itestr,bc(itestr)
       ibodcp=ibcoff                                                    3d19s25
       ibcoff=ibodcp+nuniqs                                             3d19s25
       call enough('prop.bodcp',bc,ibc)                                 3d19s25
       jbodcp=ibodcp-1                                                  3d19s25
       do i=1,nuniqs                                                    2d3s25
        write(6,*)('for dispacement direction '),i
        joder1=ioder1+noderw*(i-1)                                      2d3s25
        jvder1=ivder1+nvderw*(i-1)                                      2d3s25
        jdder1=idder1+noderw*(i-1)                                      3d12s25
        nstep=ibc(jdatal)                                               2d3s25
        write(6,*)('no. steps '),nstep,ibc(jdatal),bc(jdatal),
     $       loc(bc(jdatal)),jdatal                                     2d3s25
        ibcoff0=ibcoff                                                  2d3s25
        jdatal=jdatal+1                                                 2d3s25
        jdatal0=jdatal                                                  2d3s25
        do j=1,nstep                                                    2d3s25
         write(6,*)j,bc(jdatal),ibc(jdatal+1)
         wcoef=bc(jdatal)                                               2d3s25
         kfname=ibc(jdatal+1)                                           2d3s25
         write(6,*)('kfname = '),kfname
       write(6,*)('ibdatf: '),(ibc(jbdatf+idws),idws=1,nfname)
         if(ibc(jdatal+1).eq.1)then                                     2d3s25
          iwuse=1                                                       2d3s25
          write(6,*)('bodc1fd the first')
          call bodc1fd(iorb,iorb,iwavedat,iwavedat(1,iwuse),joder1,     2d3s25
     $         wcoef,natom,ngaus,1,kfname,ibc(ibdatf),                  2d14s25
     $         nbasdws,nbasp,multh,isym,iapair,                         2d14s25
     $         ibstor,isstor,idorel,ascale,nsymb,bc,ibc)                2d13s25
         else                                                           2d3s25
          if(mynowprog.eq.0)then                                         2d3s25
           jfname=ibc(jfnp+ibc(jdatal+1))
           nhere=ibc(jfname)                                             2d3s25
           jfname=jfname+1                                               2d3s25
           if(nhere.gt.200)then                                             11d22s19
            write(6,*)('name is tooooo long!')
            stop 'prop'
           end if                                                           11d22s19
           do k=1,nhere
            fname(k:k)=char(ibc(jfname))                                    11d22s19
            jfname=jfname+1
           end do                                                           11d22s19
           write(6,*)('file name: '),fname(1:nhere)                        11d9s22
          end if
          do isb=1,nsymb                                                2d14s25
           iorbo(isb)=ibcoff                                            2d14s25
           ibcoff=iorbo(isb)+nbasp(isb)*nbasdws(isb)*ncomp              2d14s25
           write(6,*)('space for orbitals of sym '),isb,iorbo(isb),
     $          nbasp(isb),
     $          nbasdws(isb),ncomp,ibcoff+1-iorbo(isb)
          end do                                                        2d14s25
          call enough('prop.orbo',bc,ibc)                               2d14s25
          iwuse=j+1                                                       2d3s25
          iout=ibcoff                                                      11d24s19
          call getwf(fname(1:nhere),ismult,isymmrci,nroot,iout,            2d11s22
     $       iwavedat(1,iwuse),mdoox,mxopn,iorbo,1,noff,nspc,nwavedat,  2d3s25
     $       elowest,idorel,nsymb,nbasp,nbasdws,nvirt,multh,maxbx,      7d21s21
     $      maxbxd,1,bc,ibc,nddilow,nddihig,itargs,itargd,iqhrc)        12d9s24
       write(6,*)('iorbo after getwf '),(iorbo(isb),isb=1,nsymb)
          jorbf=iorbf-1+nsymb*(iwuse-1)                                 2d3s25
          do isb=1,nsymb                                                   2d3s25
           ibc(jorbf+isb)=iorbo(isb)                                    2d3s25
          end do                                                           2d3s25
          maxbxi=maxbx+npadddi                                            7d30s22
          maxbxdi=maxbxd+npadddi                                          7d30s22
          write(6,*)('bodc1fd the second'),joder1
          write(6,*)('iwuse = '),iwuse
          call bodc1fd(iorb,iorbo,iwavedat,iwavedat(1,iwuse),joder1,    2d3s25
     $         wcoef,natom,ngaus,1,kfname,ibc(ibdatf),                  2d14s25
     $         nbasdws,nbasp,multh,isym,iapair,                         2d14s25
     $         ibstor,isstor,idorel,ascale,nsymb,bc,ibc)                2d14s25
         end if                                                         2d3s25
         do k=0,noderw-1                                                2d27s25
          bc(iden(1)+k)=0d0                                             3d12s25
         end do                                                         2d27s25
         do k=0,noderw-1                                                2d27s25
          bc(iden(1)+k)=0d0                                             3d12s25
         end do                                                         2d27s25
         write(6,*)('calling psipsiden '),iwuse,icsf,loc(bc),loc(ibc)
         call psipsiden(iwavedat(1,1),nroot,iden,iwavedat(1,iwuse),mdon,3d10s25
     $        mdooup,ixw1,ixw2,ism,irel,irefo,norb,nbasdws,idoubo,nvirt,3d10s25
     $        maxbx,maxbxd,srh,sr2,multh,nsymb,bc(icsf),bc,ibc)         3d10s25
         call dws_gsumf(bc(iden(1)),noderw)
         kdder1=jdder1                                                  2d27s25
         write(6,*)('fd weight '),wcoef
         do isb=1,nsymb                                                 2d27s25
          write(6,*)('first der density matrix for symmetry '),isb                2d27s25
          call prntm2(bc(iden(isb)),nbasdws(isb),nbasdws(isb),          2d27s25
     $         nbasdws(isb))                                            2d27s25
          do k=0,noderw-1                                               2d27s25
           bc(kdder1+k)=bc(kdder1+k)+wcoef*bc(iden(isb)+k)              3d12s25
          end do                                                        2d27s25
          write(6,*)('fd den matrix so far ')                           3d28s25
          call prntm2(bc(kdder1),nbasdws(isb),nbasdws(isb),             2d27s25
     $         nbasdws(isb))                                            2d27s25
          kdder1=kdder1+nbasdws(isb)*nbasdws(isb)                       2d27s25
         end do                                                         2d27s25
         call dovr(iwavedat(1,1),iwavedat(1,iwuse),bc(idovr),mdon,
     $       mdooup,ixw1,ixw2,ism,irel,irefo,norb,nbasdws,idoubo,nvirt,
     $       ibc(ibcmaxbx),
     $       maxbxd,srh,sr2,multh,nsymb,bc(icsf),npadddi,bc,ibc)        11d10s22
         write(6,*)('vector overlap ')
         call prntm2(bc(idovr),nroot,nroot,nroot)
         do k=0,nvderw-1                                                2d3s25
          bc(jvder1+k)=bc(jvder1+k)+wcoef*bc(idovr+k)                   2d3s25
         end do                                                         2d3s25
         write(6,*)('fd matrix so far ')                                2d3s25
         call prntm2(bc(jvder1),nroot,nroot,nroot)                      2d3s25
         jdatal=jdatal+2
        end do                                                          2d3s25
        write(6,*)('computing suma (I think!)')
        suma=0d0                                                        3d14s25
        kdder1=jdder1                                                   3d14s25
        koder1=joder1                                                   3d14s25
        bodc=0d0                                                        3d14s25
        sum2=0d0                                                        3d17s25
        sumk=0d0                                                        3d17s25
        do isb=1,nsymb
         write(6,*)('suma for symmetry block '),isb                     3d14s25
         do ia=0,nbasdws(isb)-1                                         3d14s25
          do ib=0,nbasdws(isb)-1                                        3d14s25
           iad1=kdder1+ib+nbasdws(isb)*ia                               3d14s25
           iad2=kdder1+ia+nbasdws(isb)*ib                               3d14s25
           iad3=koder1+ia+nbasdws(isb)*ib                               3d14s25
           suma=suma+bc(iad3)*(bc(iad2)-bc(iad1))                       3d14s25
          end do                                                        3d14s25
         end do                                                         3d14s25
         write(6,*)('suma so far '),suma,suma*0.5d0
         do ia=0,idoubo(isb)-1                                          3d17s25
          do ib=0,idoubo(isb)-1                                         3d17s25
           iab=koder1+ib+nbasdws(isb)*ia                                3d17s25
           sumk=sumk+bc(iab)**2                                         3d17s25
          end do                                                        3d17s25
          do k=idoubo(isb),nbasdws(isb)-1                               3d17s25
           ki=koder1+k+nbasdws(isb)*ia                                  3d17s25
           do l=idoubo(isb),nbasdws(isb)-1                              3d17s25
            lk=ident(isb)+l+nbasdws(isb)*k                              3d17s25
            li=koder1+l+nbasdws(isb)*ia                                 3d17s25
            orig=sum2
            sum2=sum2+bc(ki)*bc(li)*bc(lk)                              3d17s25
            if(abs(sum2-orig).gt.1d-10)write(6,*)orig,bc(ki),bc(li),
     $           bc(lk),sum2
           end do                                                       3d17s25
          end do                                                        3d17s25
         end do                                                         3d17s25
         write(6,*)('sumk so far '),sumk                                3d17s25
         write(6,*)('sum2 so far '),sum2                                3d17s25
         kdder1=kdder1+nbasdws(isb)*nbasdws(isb)                        3d14s25
         koder1=koder1+nbasdws(isb)*nbasdws(isb)                        3d14s25
        end do                                                          3d14s25
        bodc=suma*0.5d0                                                 3d14s25
        write(6,*)('bodc after suma '),bodc
        joder2=ioder2+noderw*(i-1)                                      2d3s25
        jvder2=ivder2+nvderw*(i-1)                                      2d3s25
        jdder2=idder2+noderw*(i-1)                                      3d12s25
        write(6,*)('for second derivative ...')
        jbdatal=jdatal0                                                 2d3s25
        do jb=1,nstep                                                   2d3s25
         wcoefb=bc(jbdatal)                                             2d3s25
         lfnameb=ibc(jbdatal+1)                                         2d3s25
         if(lfnameb.eq.1)then                                           2d3s25
          iwbuse=1                                                      2d3s25
         else                                                           2d3s25
          iwbuse=jb+1                                                   3d14s25
         end if                                                         2d3s25
         jkdatal=jdatal0                                                2d3s25
         do jk=1,nstep                                                  2d3s25
          wcoefk=bc(jkdatal)                                            2d3s25
          lfnamek=ibc(jkdatal+1)                                        2d3s25
          if(lfnamek.eq.1)then                                          2d3s25
           iwkuse=1                                                     2d3s25
          else                                                          2d3s25
           iwkuse=jk+1                                                  3d14s25
          end if                                                        2d3s25
          wcoefp=wcoefb*wcoefk                                          2d3s25
          write(6,*)('bra step '),jb,('ket step '),jk,wcoefb,wcoefk,
     $         wcoefp,jb,jk                                             2d3s25
          jkorbf=iorbf-1+nsymb*(iwkuse-1)                                 2d3s25
          jborbf=iorbf-1+nsymb*(iwbuse-1)                               2d3s25
          do isb=1,nsymb                                                   2d3s25
           iorb(isb)=ibc(jborbf+isb)                                    2d3s25
           iorbo(isb)=ibc(jkorbf+isb)                                   2d3s25
          end do                                                           2d3s25
          write(6,*)('bodc1fd the third')
          call bodc1fd(iorb,iorbo,iwavedat(1,iwbuse),iwavedat(1,iwkuse),
     $         joder2,wcoefp,natom,ngaus,lfnameb,lfnamek,ibc(ibdatf),   2d14s25
     $         nbasdws,nbasp,multh,isym,iapair,                         2d14s25
     $         ibstor,isstor,idorel,ascale,nsymb,bc,ibc)                2d14s25
          do k=0,noderw-1                                                2d27s25
           bc(iden(1)+k)=0d0                                             3d12s25
          end do                                                         2d27s25
          write(6,*)('iwbuse,iwkuse '),iwbuse,iwkuse
          call psipsiden(iwavedat(1,iwbuse),nroot,iden,                 3d12s25
     $         iwavedat(1,iwkuse),mdon,mdooup,ixw1,ixw2,ism,irel,irefo,  3d12s25
     $         norb,nbasdws,idoubo,nvirt,maxbx,maxbxd,srh,sr2,multh,    3d12s25
     $         nsymb,bc(icsf),bc,ibc)                                   3d12s25
          call dws_gsumf(bc(iden(1)),noderw)
          kdder2=jdder2                                                  2d27s25
          do isb=1,nsymb                                                 2d27s25
           write(6,*)('2nd der density matrix for symmetry '),isb                2d27s25
           call prntm2(bc(iden(isb)),nbasdws(isb),nbasdws(isb),          2d27s25
     $         nbasdws(isb))                                            2d27s25
           do k=0,noderw-1                                               2d27s25
            bc(kdder2+k)=bc(kdder2+k)+wcoefp*bc(iden(isb)+k)              2d27s25
           end do                                                        2d27s25
           write(6,*)('fd matrix so far ')
           call prntm2(bc(kdder2),nbasdws(isb),nbasdws(isb),             2d27s25
     $         nbasdws(isb))                                            2d27s25
           kdder2=kdder2+nbasdws(isb)*nbasdws(isb)                       2d27s25
          end do                                                         2d27s25
          call dovr(iwavedat(1,iwbuse),iwavedat(1,iwkuse),bc(idovr),    2d3s25
     $         mdon,mdooup,ixw1,ixw2,ism,irel,irefo,norb,nbasdws,idoubo,2d3s25
     $         nvirt,ibc(ibcmaxbx),                                     2d3s25
     $       maxbxd,srh,sr2,multh,nsymb,bc(icsf),npadddi,bc,ibc)        11d10s22
          write(6,*)('vector overlap ')
          call prntm2(bc(idovr),nroot,nroot,nroot)
          do k=0,nvderw-1                                               2d3s25
           bc(jvder2+k)=bc(jvder2+k)+wcoefp*bc(idovr+k)                  2d3s25
          end do                                                        2d3s25
          write(6,*)('fd matrix so far ')                               2d3s25
          call prntm2(bc(jvder2),nroot,nroot,nroot)                     2d3s25
          jkdatal=jkdatal+2                                             2d3s25
         end do                                                         2d3s25
         jbdatal=jbdatal+2                                              2d3s25
        end do                                                          2d3s25
        write(6,*)('computing dddd part (I think?)')
        ddorbp=0d0                                                      3d14s25
        ddci=0d0
        write(6,*)('dndm*den part ')                                    3d14s25
        koder2=joder2                                                   3d14s25
        kdder2=jdder2                                                   3d14s25
        dddd=0d0                                                        3d14s25
        do isb=1,nsymb                                                  3d14s25
         write(6,*)('for symmetry block '),isb
         do ia=0,idoubo(isb)-1                                          3d14s25
          iad=koder2+ia*(nbasdws(isb)+1)                                3d14s25
          dddd=dddd+bc(iad)                                             3d14s25
         end do                                                         3d14s25
         write(6,*)('dddd so far '),dddd
         do ia=idoubo(isb),nbasdws(isb)-1                               3d17s25
          do ib=idoubo(isb),nbasdws(isb)-1                              3d17s25
           iab=ib+nbasdws(isb)*ia                                       3d17s25
           ddorbp=ddorbp+bc(ident(isb)+iab)*bc(koder2+iab)              3d17s25
          end do                                                        3d17s25
         end do                                                         3d17s25
         write(6,*)('ddorbp part so far '),ddorbp,ddorbp*0.5d0
         do ia=0,nbasdws(isb)-1                                         3d14s25
          iad=kdder2+ia*(nbasdws(isb)+1)                                3d14s25
          ddci=ddci+bc(iad)                                             3d14s25
         end do                                                         3d14s25
         write(6,*)('ddci part so far '),ddci,ddci*0.5d0
         kdder2=kdder2+nbasdws(isb)*nbasdws(isb)                        3d14s25
         koder2=koder2+nbasdws(isb)*nbasdws(isb)                        3d14s25
        end do                                                          3d14s25
        bodc=bodc+dddd+(ddorbp+ddci)*0.5d0                              3d14s25
        write(6,*)('one e part of bodc: '),bodc                         3d14s25
        bodc=bodc-sum2                                                  3d17s25
        write(6,*)('bodc less sum2 '),bodc                              3d17s25
        bodc=bodc-sumk                                                  3d17s25
        write(6,*)('bodc less sumk '),bodc                              3d17s25
        bc(jbodcp+i)=bodc                                               3d19s25
        ibcoff=ibcoff0                                                  2d3s25
       end do                                                           2d3s25
       write(6,*)('now move on to 2e- contributions ')
       call make1dm(nsymb,irefo,nbasdws,lprint,maxddi,mjmat,mkmat,      3d26s25
     $      mhmat,jdenpt,isblk,isblkk,isblkh,nsdlk,nsdlkk,nsdlkh,multh, 3d26s25
     $      idbk,isblk1,nsdlk1,isblkd,nsdlkd,0)                         3d26s25
       write(6,*)('calling denmx12bodc with ioder1 = '),ioder1
       call denmx12bodc(bc,ibc,nh0,idoubo,irefo,nbasdws,ibc(icsf),      3d26s25
     $      iwavedat,mdon,mdooup,ixw1,ixw2,maxbx,maxbxd,srh,sr2,multh,  3d26s25
     $      norb,nuniqs,bc(ibodcp),bc(ioder1),noderw,ism,irel)          3d26s25
       return                                                           2d3s25
      end if                                                            2d3s25
      xmasstot=0d0                                                      3d31s22
      maxbx=0                                                           7d9s21
...
c
     ...
     $       maxbxd,srh,sr2,multh,nsymb,bc(icsf),npadddi,bc,ibc)        11d10s22
       ...
       sz=0d0                                                           2d11s22
       if(nroot.eq.1)go to 2258                                         2d14s25
       numb=0                                                           2d11s22
 ...
 2258  continue                                                         2d11s22
       ...
       if(iqhrc.ne.0.and.lprint)then                                    12d9s24
        write(6,*)('using +q(h,RD) energies ')                          12d9s24
       end if                                                           12d9s24
       do io=0,nroot-1
...
c     tm is itmoo, tmb is dum
     ...
     $       bc(itmoe),ndimo,lprint,bc,ibc,iqhrc)                       12d9s24
     ...
     $       bc(itmeo),ndime,lprint,bc,ibc,iqhrc)                       12d9s24
