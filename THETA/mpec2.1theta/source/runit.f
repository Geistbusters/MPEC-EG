c mpec2.1 version theta. For copyright and Disclaimers, see start.f
     ...
     $     'B3g','Au '/                                                 4d28s10
      ...
      save
      integer*8 ibcgh,ibcprior,ibcrys,ibdat,ibstor,idenergy,iextradatad
     $isend1,iwavedat
      drsign=1d0                                                        8d20s24
      iagrp(1)=0                                                        5d26s21
      ioffsx=1                                                          11d9s22
 ...
 1066  continue                                                         2d27s19
     ...
     $      ibc,cfile,ncfile)                                           12d13s22
      ...
      maxbct=nint(pass(2),8)                                            2e19G26-1d31s21
...
c
      if(mynowprog.eq.0)then
       ...
       write(6,*) 'DEBUG1: nqx=',nqx,' nwrys=',nwrys,' mxrysi=',mxrysi  EGD
       write(6,*) 'DEBUG1: ibcrys=',ibcrys,' ibcoff=',ibcoff            EGD

       write(6,*)('maximum order of rys quadrature '),nqx               6d22s12
       ibcrys=ibcoff                                                    6d22s12
       ibcoff=ibcoff+nwrys+2*nqx                                        6d22s12
       
       write(6,*) 'DEBUG1.5: maxbct=',maxbct,' ibcoff=',ibcoff
       
       call enough('start.  1',bc,ibc)
       write(6,*) 'DEBUG1.6: About to read ibc array'
       read(1)(ibc(ibcrys+j),j=0,2*nqx-1)                               6d22s12
       write(6,*) 'DEBUG1.7: Done reading ibc, about to read bc array'
       read(1)(bc(ibcrys+2*nqx+j),j=0,nwrys-1)                          6d22s12
       write(6,*) 'DEBUG1.8: Done reading bc array'
         write(6,*) 'DEBUG2: First ibc values:',
     $      (ibc(ibcrys+j),j=0,min(5,2*nqx-1))
       write(6,*) 'DEBUG2: First bc values:',
     $      (bc(ibcrys+2*nqx+j),j=0,min(5,nwrys-1))
  ...
  447  format(' scratch memory space: ',i15,(' words or '),f5.1,x,       12d2s22
     $      'Gbytes.')                                                  12d2s22
      end if                                                            2d16s10
