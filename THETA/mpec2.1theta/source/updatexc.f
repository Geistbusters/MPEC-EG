...
c
c     start out by normalizing so we can judge how much is left after
c     projecting out old data.
c
       sum2=0d0                                                         11d10s20
       if(ndoub.gt.0)then                                               2d17s21
        ioff=0                                                           11d10s20
       ...
       if(lprt)then                                                     1d28s25
        write(6,*)('length of trial vector '),ir,dot
       end if
          ...
          if(jsbv2.ge.jsbv1)then
           if(jsbv12.eq.1.and.nvirt(jsbv1).gt.0)then
            ...
            do if=1,nfdat(3,1,isymmrci)                                   11d10s20
             joff=joff+nvirt(jsbv1)*(ir-1)                               11d10s20
             do iv=1,nvirt(jsbv1)                                        11d10s20
              vdinout(joff+iv)=vdinout(joff+iv)*xnorm                   11d27s24
             end do                                                      11d10s20
             joff=joff+nvirt(jsbv1)*(nroot+1-ir)                         11d10s20
            end do                                                       11d10s20
            ioff=ioff+nvirt(jsbv1)*nn                                    11d10s20
            ...
            do l=1,4
             if(nfdat(3,l,jsb).gt.0)then
              ...
              do if=1,nfdat(3,l,jsb)                                      11d10s20
               joff=joff+nvv*(ir-1)                                      11d10s20
               do iv12=1,nvv                                             11d10s20
                vdinout(joff+iv12)=vdinout(joff+iv12)*xnorm             11d27s24
               end do                                                    11d10s20
               joff=joff+nvv*(nroot+1-ir)                                11d10s20
              end do                                                     11d10s20
              ioff=ioff+nvv*nn                                           11d10s20
       ...
       if(nsing.gt.0)then                                               3d19s21
        ...
        do isb=1,nsymb                                                   11d10s20
         ...
         do ii=mdon+1,mdoo+1                                             11d10s20
          ...
          if(ndblock.gt.0)then                                           11d10s20
           icol4=ndblock                                                 11d10s20
           call ilimts(1,icol4,mynprocg,mynowprog,il,ih,i1s,i1e,i2s,    11d27s24
     $           i2e)                                                   11d27s24
           ic=0                                                           11d10s20
           do iff=1,nff1(ii,isb)                                         11d10s20
            do m=1,ncsf(iarg)                                            11d10s20
             ...
             if(ic.ge.il.and.ic.le.ih)then                               11d10s20
              jvstryu=jvstry+ir                                          12d14s20
              do iv=1,nvirt(isbv)                                        11d10s20
               vstry(jvstryu)=vstry(jvstryu)*xnorm                      11d27s24
               jvstryu=jvstryu+nroot                                     12d14s20
              end do                                                     11d10s20
              jvstry=jvstry+nvirt(isbv)*nroot                            12d14s20
             end if                                                      11d10s20
       ...
       call dws_sync                                                    11d27s24
       ireorth=0                                                        11d27s24
 1492  continue                                                         11d27s24
       ireorth=ireorth+1                                                11d27s24
...
c                                                                       11d27s24
c     now normalize so we can check if we need to redo orthogonalization11d27s24
c                                                                       11d27s24
       sum2=0d0                                                         11d10s20
       if(ndoub.gt.0)then                                               2d17s21
        ioff=0                                                           11d10s20
       ...
       dot=sum1+sum2                                                    11d10s20
       if(lprt)then                                                     1d28s25
        write(6,*)('length of half orthogonalized vector '),ir,dot
       end if
       xnorm=1d0/sqrt(dot)                                              11d10s20
       if(dot.lt.1d-7)then                                              11d27s24
        if(lprt)write(6,*)('normalize and re-orthogonalize ')           1d28s25
        if(ireorth.gt.5)then
         write(6,*)('failed to  orthogonalize trial vectors')           3d25s25
         write(6,*)('we''ve already tried toooo many times!')           11d27s24
         call dws_synca                                                 11d27s24
         call dws_finalize                                              11d27s24
         stop 'updatexc'                                                11d27s24
        end if                                                          11d27s24
        if(ndoub.gt.0)then                                               2d17s21
         ioff=0                                                           11d10s20
         do jsb=1,nsymb                                                    12d8s20
...
c
       ...
       dot=sum1+sum2                                                    11d10s20
       if(lprt)then                                                     1d28s25
        write(6,*)('length of orthogonalized vector '),ir,dot
       end if
       xnorm=1d0/sqrt(dot)                                              11d10s20
       if(lprt)then                                                     1d25s21
        write(6,*)('for root '),ir
