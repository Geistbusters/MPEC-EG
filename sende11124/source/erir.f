...
c
 ...
 1066 continue
      if(idoit.eq.1.or.idbg.ne.0)then
      write(6,1)la,zza,xa,ya,za,lb,zzb,xb,yb,zb,lc,zzc,xc,yc,zc,        1d29s10
     $     ld,zzd,xd,yd,zd,icall                                        7d22s14
      idbg=1
      end if
    1 format('in erir: ',4(i3,1pe15.7,0p3f8.4),i8)                       7d22s14
      ltmp(1)=la+1                                                      1d13s10
      ltmp(2)=lb+1                                                      1d13s10
      ltmp(3)=lc+1                                                      1d13s10
      ltmp(4)=ld+1                                                      1d13s10
   ...
   45 format(i5,f10.5)
     ...
     $     ('available '),nqr,nqx                                       6d22s12
      ...
      if(idbg.ne.0)then
       write(6,*)('iaddq, nreg '),iaddq,nreg,ibcrys+1+2*(nqr-1),ibcrys,
     $      nqr
      end if
      ...
      if(idbg.ne.0)then
       ...
       write(6,*)('bigx,iaddq '),bigx,bc(iaddq),iaddq
      end if
      rmsx=0d0
      rmsw=0d0
      if(ireg.le.nreg)then                                              6d22s12
       ...
       if(idbg.ne.0)then
        write(6,*)('try, itry '),try,itry,iaddq+2*nqr+ireg
       end if
       do i=1,nqr                                                       6d22s12
        ...
        do j=ncoef,2,-1                                                 6d22s12
         ...
         svw=dw                                                         6d22s12
         svn=dn                                                         6d22s12
         if(idbg.ne.0)write(6,*)j,j1,j2,bc(j1),bc(j2)
        ...
        if(idbg.ne.0)then
         write(6,*)('xnod '),xnod,y,dn,ddn,j2,bc(j2)
        end if
         ...
         do ic=0,lc
          do ib=0,lb
           do ia=0,la
            ...
            iad2=ixr+iq-1+nqr*(ia+lapp*(ib+lbpp*(ic+lcpp*id)))
            bc(iad1)=bc(iad2)
           end do
          end do
         end do
        ...
        do id=0,ld
         do ic=0,lc
          do ib=0,lb
           do ia=0,la
            iad1=itmp+ia+lap*(ib+lbp*(ic+lcp*id))
            iad2=iyr+iq-1+nqr*(ia+lapp*(ib+lbpp*(ic+lcpp*id)))
            bc(iad1)=bc(iad2)
           end do
          end do
         end do
        ...
        do id=0,ld
         do ic=0,lc
          do ib=0,lb
           do ia=0,la
            iad1=itmp+ia+lap*(ib+lbp*(ic+lcp*id))
            iad2=izr+iq-1+nqr*(ia+lapp*(ib+lbpp*(ic+lcpp*id)))
...
c     we have
   ...
   50 format(4i5,1pe26.18)
      ...
      do ia=1,lat                                                       1d29s10
       do ib=1,lbt                                                      1d29s10
        do ic=1,lct                                                     1d29s10
         do id=1,ldt                                                    1d29s10
          rmsz=rmsz+bc(jrij)**2
          if(bc(jrij).ne.bc(jrij).and.idbg.eq.0)then                    4d25s18
      ...
      if(idbg.ne.0.and.idoit.ne.1)then                                  10d28s20
       call dws_sync                                                    4d25s18
       call dws_finalize                                                4d25s18
       stop                                                             4d25s18
      end if                                                            4d25s18
      return
      end
