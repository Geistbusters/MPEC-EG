c mpec2.1 version eta. For copyright and Disclaimers, see start.f
      subroutine paraerid2b(natom,ngaus,ibdat,nbasis,ihmat,iorb,nocc,   9d28s16
...
c
c     like paraerid, except form (a'b'|cd)                              9d28s16
...
c
      ...
      memneed(1:3)='i12'
      call enough('paraerid2b.  1',bc,ibc)
      j12=i12                                                           2d19s10
...
c
 ...
 3353    format(3i3,5x,3i3,5x,i8)
          ...
          do isd=1,nsymb                                                 5d12s10
           ...
           if(nadd.gt.0)then                                             1d30s12
            nadd=nadd*ntmpmat                                           3d6s23
            if(iptoh(isd,isc,isb).gt.0)then                              9d20s16
             nhcol(iptoh(isd,isc,isb))=nhcol(iptoh(isd,isc,isb))+nadd   9d28s16
             nsum1=nsum1+1
            else if(iptoh(isd,isc,isa).gt.0)then                        9d28s16
             nhcol(iptoh(isd,isc,isa))=nhcol(iptoh(isd,isc,isa))+nadd   9d28s16
             nsum1=nsum1+1
            end if                                                       1d30s12
           end if                                                        1d30s12
          end do                                                         5d12s10
     ...
     $     (' elements of hmat')                                        3d15s12
      ...
      memneed(1:4)='hmat'
      call enough('paraerid2b.  2',bc,ibc)
      ...
      call enough('paraerid2b.  3',bc,ibc)
 ...
 3352  format(6i5)
       ...
       memneed(1:5)='tmpab'
       call enough('paraerid2b.  4',bc,ibc)
       ...
       memneed(1:4)='eraw'
       call enough('paraerid2b.  5',bc,ibc)
        ...
        do ipassa=1,nszaa/nsza                                          5d12s10
         ...
         do igc=1,ngaus                                                 3d2s16
          do igd=1,igc                                                  9d28s16
           ...
           memneed(1:4)='itmp'
           call enough('paraerid2b.  6',bc,ibc)
           do iz=itmp,ibcoff-1                                          5d31s22
            bc(iz)=0d0                                                  5d31s22
           end do                                                       5d31s22
           nwds=nsza*nszb*nszc*nszd
           ldeb=idoit.eq.1                                              4d21s16
           if(ibc(ja8).eq.iatom.and.ibc(jb8).eq.iatom                   3d3s23
     $          .and.ipassa.eq.ipassb)then                              3d3s23
            if(idorel.eq.0)then                                         3d6s23
     ...
     $             ieri,0,sigab,idx,idy,idz,idx,idy,idz,0,0,0,0,0,0,bc, 11d14s22
     $          ibc)                                                    11d14s22
            else                                                        3d6s23
              call restind                                              3d6s23
     ...
     $             ibc(jc),bc(jc2),bc(jc3),bc(jc5),bc(jc6),bc(jc7),     3d6s17
     $             ibc(jd),bc(jd2),bc(jd3),bc(jd5),bc(jd6),bc(jd7),     3d6s17
     $             ieri,0,sigab,idorel,ascale,nmat,idx,idy,idz,idx,idy, 3d6s23
     $            idz,0,0,0,0,0,0,bc,ibc)                               3d6s23
             end if                                                     3d6s23
           else                                                          2d23s16
            ieri=ibcoff
            ibcoff=ieri+nwds                                            4d21s16
            call enough('paraerid2b.  7',bc,ibc)
            do k=0,nwds-1
             bc(ieri+k)=0d0
            end do
           end if
...
c
     ...
     $           .and.ipassa.eq.ipassb)then                             3d3s23
             if(idorel.eq.0)then                                        3d6s23
              call erird(ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3)3d6s23
     $            ,ibc(jb),bc(jb2),bc(jb3),cartb,cartb(2),cartb(3),     3d6s23
     $         ibc(jc),bc(jc2),bc(jc3),cartc,cartc(2),cartc(3),         3d6s23
     $         ibc(jd),bc(jd2),bc(jd3),bc(jd5),bc(jd6),bc(jd7),         3d6s23
     $         ieri,0,sigab,idx,idy,idz,idx,idy,idz,                    3d6s23
     $           0,0,0,0,0,0,bc,ibc)                                    3d6s23
             else                                                       3d6s23
              call restind
     $             (ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3)     3d6s23
     $            ,ibc(jb),bc(jb2),bc(jb3),cartb,cartb(2),cartb(3),     3d6s23
     $         ibc(jc),bc(jc2),bc(jc3),cartc,cartc(2),cartc(3),         3d6s23
     $         ibc(jd),bc(jd2),bc(jd3),bc(jd5),bc(jd6),bc(jd7),         3d6s23
     $         ieri,0,sigab,idorel,ascale,nmat,idx,idy,idz,idx,idy,idz, 3d6s23
     $           0,0,0,0,0,0,bc,ibc)                                    3d6s23
             end if                                                     3d6s23
            else                                                        4d18s16
             ieri=ibcoff
             ibcoff=ieri+nwds                                           4d21s16
             call enough('paraerid2b.  8',bc,ibc)
             do k=0,nwds-1
              bc(ieri+k)=0d0
             end do
            end if
     ...
     $           .and.ipassa.eq.ipassb)then                             3d3s23
             if(idorel.eq.0)then                                        3d6s23
              call erird(ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3)3d6s23
     $          ,ibc(jb),bc(jb2),bc(jb3),cartb,cartb(2),cartb(3),       3d6s23
     $         ibc(jc),bc(jc2),bc(jc3),bc(jc5),bc(jc6),bc(jc7),         3d6s23
     $         ibc(jd),bc(jd2),bc(jd3),cartd,cartd(2),cartd(3),         3d6s23
     $         ieri,0,sigab,idx,idy,idz,idx,idy,idz,                    3d6s23
     $           0,0,0,0,0,0,bc,ibc)                                    3d6s23
             else                                                       3d6s23
              call restind                                              3d6s23
     ...
     $           0,0,0,0,0,0,bc,ibc)                                    3d6s23
             ...
             ieri=ibcoff
             ibcoff=ieri+nwds                                           4d21s16
             call enough('paraerid2b.  9',bc,ibc)
             do k=0,nwds-1
              bc(ieri+k)=0d0
             end do
            end if
            ...
            ibcoff=ieri                                                 4d21s16
            if(iapair(1,ibc(jc8)).gt.0)then                                5d12s10
             if(ibc(ja8).eq.iatom.and.ibc(jb8).eq.iatom
     $           .and.ipassa.eq.ipassb)then                             3d3s23
              if(idorel.eq.0)then                                       3d6s23
               call erird                                               3d6s23
     $             (ibc(ja),bc(ja2),bc(ja3),carta,carta(2),carta(3),    3d6s23
     ...
     $            0,0,0,0,0,0,bc,ibc)                                   3d6s23
              ...
              ieri=ibcoff
             ibcoff=ieri+nwds                                           4d21s16
             call enough('paraerid2b. 10',bc,ibc)
              do k=0,nwds-1
               bc(ieri+k)=0d0
              end do
             end if
     ...
     $            +nbasisp(isc)*in))                                    10d28s20
              ...
              do iab=0,ncsz0-1                                          6d2s22
               ...
               bc(iad1b+iab)=bc(iad2+iab)                                9d28s16
              end do                                                       5d12s10
             end do                                                     3d6s23
            end do                                                        5d12s10
           end do                                                         5d12s10
...
c     1d10s11
          ...
          if(nocc(isd).gt.0)then                                        6d2s10
           do isc=1,nsymb                                                 5d12s10
            if(nbasdws(isc).gt.0)then                                   9d28s16
             ...
             memneed(1:5)='ihalf'
             call enough('paraerid2b. 11',bc,ibc)
             if(idoit.eq.1)then
     ...
     $             nbasisp(isd)*ncomp)                                  4d7s22
              write(6,*)('integrals ')
              nnz=0d0
              do ixd=1,nabc*nbasisp(isd)*nerimat                        3d7s23
               if(abs(bc(ieraw(isc,isd)+ixd-1)).gt.1d-6)nnz=nnz+1       9d20s16
              end do
              write(6,*)('nonzero: '),nnz,(' vs '),ndoit
              call prntm2(bc(ieraw(isc,isd)),nabc,nbasisp(isd)*nerimat, 3d7s23
     $             nabc)
             end if
